...
void check(int zona[5][15], int luces[5], int maxcalle[5]) {
    ...
    for(int i = 0; i < 5; i++) {
        for(int j = 0; j < 15; j++) { //calcula la cantidad de luces apagadas por cada calle
            if(zona[i][j] == 0) lucesApagadas++;
        }
    }
    for(int i = 0; i < 5; i++) {
        cout << "Calle " << i+1 << ":";
    if(lucesApagadas == 15) cout << "Oscura" << endl; //si todas las luces estan apagadas, la calle es tenebrosa
    else if(luces[i] == maxcalle[i] && luces[i] > lucesApagadas) cout << "Muy luminosa" << endl; //si hay misma cantidad
    //de luces prendidas como de maximo de luces y las luces prendidas es mayor a las luces apagadas, es una calle muy luminosa
    else if(luces[i] == maxcalle[i] && luces[i] <= lucesApagadas) cout << "Luminosa" << endl; //si hay misma cantidad
    ...
    while(1) {
        ...
        if(m <= 5 && m > 0 && l <= 15 && l > 0) {
            if(luces[m-1] < maxcalle[m-1]) {
                ...
                cout << "Llego al maximo de luces" << endl;
                printZona(zona);
            } 
